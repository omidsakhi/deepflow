...
std::shared_ptr<Session> create_generator_session() {
	...
	auto gin = df.data_generator(df.random_uniform({ FLAGS_batch, 100, 1, 1 }, -1.0, 1.0, "random_input"), 10000, "", "input");
	...
	//auto gfc_b = df.batch_normalization(gfc, DeepFlow::PER_ACTIVATION, 0, 1, 0, alpha_param, 1 - decay);
...
std::shared_ptr<Session> create_discriminator() {
	...
	auto mean = 0;
	auto stddev = 0.02;
	auto d_solver = df.adam_solver(0.0001f, 0.5f, 0.999f);
	auto negative_slope = 0.01;
	auto dropout = 0.5; 
	auto depth = 64; 
	...
	auto relu1 = df.leaky_relu(bias1, negative_slope);
	auto drop1 = df.dropout(relu1, dropout);
