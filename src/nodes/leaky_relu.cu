...
LeakyRelu::LeakyRelu(const deepflow::NodeParam &_block_param) : Node(_block_param) {
	LOG_IF(FATAL, _block_param.has_leaky_relu_param() == false) << "param.has_leaky_relu_param() == false";	
}
