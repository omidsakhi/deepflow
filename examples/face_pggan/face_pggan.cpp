...
std::string deconv(DeepFlow *df, std::string input, std::string solver, int input_channels, int output_channels, int kernel, int pad, std::string name) {
	...
	auto node = df->transposed_conv2d(input, input_channels, output_channels, solver, ConvolutionOp(name + "_tconv").kernel(kernel).pad(pad).stride(2));
	return df->batch_normalization(node, output_channels, solver, BatchNormalizationOp(name + "_tconv_bn").exponent_factor(0.001f));
}

std::string to_rgb(DeepFlow *df, std::string input, std::string solver, int input_channels, std::string name) {
	return df->conv2d(input, input_channels, 3, solver, ConvolutionOp(name + "_conv").kernel(1).pad(0).stride(1).with_bias());
}

std::string from_rgb(DeepFlow *df, std::string input, std::string solver, int output_channels, std::string name) {
	return df->conv2d(input, 3, output_channels, solver, ConvolutionOp(name + "_conv").kernel(1).pad(0).stride(1).with_bias());
}
...
std::string conv(DeepFlow *df, std::string input, std::string solver, int input_channels, int output_channels, int kernel, int pad, bool activation, std::string name) {
	auto node = df->conv2d(input, input_channels, output_channels,solver, ConvolutionOp(name + "_conv").kernel(kernel).pad(pad).stride(2));
	node = df->batch_normalization(node, output_channels, solver, BatchNormalizationOp(name + "_conv_bn").exponent_factor(0.001f));
	if (activation) {
		return df->prelu(node, output_channels, solver, PReluOp(name + "_relu"));
	}
	return node;
}
...
std::shared_ptr<Session> create() {
	...
	auto g_solver = df.adam_solver( AdamSolverOp("g_adam").lr(0.0002f).beta1(0.5f).beta2(0.98f) );	
	auto d_solver = df.adam_solver( AdamSolverOp("d_adam").lr(0.0002f).beta1(0.5f).beta2(0.98f) );

	// The input for generator when needed
	auto z = df.data_generator(df.random_normal({ FLAGS_batch, 100, 1, 1 }, 0, 1), DataGeneratorOp("z"));	
	
	auto face_data_128 = df.imbatch(FLAGS_faces, { FLAGS_batch, FLAGS_channels, FLAGS_size, FLAGS_size }, ImbatchOp("face_data_128"));
	auto face_data_64 = df.resize(face_data_128, 0.5, 0.5, ResizeOp("face_data_64"));
	auto face_data_32 = df.resize(face_data_64, 0.5, 0.5, ResizeOp("face_data_32"));
	auto face_data_16 = df.resize(face_data_32, 0.5, 0.5, ResizeOp("face_data_16"));
	auto face_data_8 = df.resize(face_data_16, 0.5, 0.5, ResizeOp("face_data_8"));
	
	auto face_labels = df.data_generator(df.fill({ FLAGS_batch, 1, 1, 1 }, 1), DataGeneratorOp("face_labels"));
	auto generator_labels = df.data_generator(df.fill({ FLAGS_batch, 1, 1, 1 }, 0), DataGeneratorOp("generator_labels"));

	auto g4 = df.dense(z, { 100, fn , 4 , 4 }, g_solver, DenseOp("g4")); // 4x4

	auto g8 = deconv(&df, g4, g_solver, fn, fn, 3, 1, "g8"); // 8x8
	auto g8elu = df.leaky_relu(g8, LeakyReluOp("g8elu"));
	auto g8trgb = to_rgb(&df, g8, g_solver, fn, "g8trgb");

	auto g16 = deconv(&df, g8elu, g_solver, fn, fn, 3, 1, "g16"); // 16x16
	auto g16elu = df.leaky_relu(g16, LeakyReluOp("g16elu"));
	auto g16trgb = to_rgb(&df, g16, g_solver, fn, "g16trgb");
	auto g16add = df.add(df.resize(g8trgb, 2, 2), g16trgb, AddOp("g16add"));
	auto im16 = df.switcher(g16trgb, SwitchOp("im16"));
	df.imwrite(im16, "i16-{it}", ImwriteOp("imw16"));

	auto g32 = deconv(&df, g16elu, g_solver, fn, fn, 3, 1, "g32"); // 32x32
	auto g32elu = df.leaky_relu(g32, LeakyReluOp("g32elu"));
	auto g32trgb = to_rgb(&df, g32, g_solver, fn, "g32trgb");
	auto g32add = df.add(df.resize(g16add, 2, 2), g32trgb, AddOp("g32add"));
	auto im32 = df.switcher(g32trgb, SwitchOp("im32"));
	df.imwrite(im32, "i32-{it}", ImwriteOp("imw32"));

	auto g64 = deconv(&df, g32elu, g_solver, fn, fn, 3, 1, "g64"); // 64x64
	auto g64elu = df.leaky_relu(g64, LeakyReluOp("g64elu"));
	auto g64trgb = to_rgb(&df, g64, g_solver, fn, "g64trgb");
	auto g64add = df.add(df.resize(g32add, 2, 2), g64trgb, AddOp("g64add"));
	auto im64 = df.switcher(g64trgb, SwitchOp("im64"));
	df.imwrite(im64, "i64-{it}", ImwriteOp("imw64"));

	auto g128 = deconv(&df, g64elu, g_solver, fn, fn, 3, 1, "g128"); // 64x64 -> 128x128
	auto g128trgb = to_rgb(&df, g128, g_solver, fn, "g128trgb");
	auto g128add = df.add(df.resize(g64add, 2, 2), g128trgb, AddOp("g128add"));
	auto im128 = df.switcher(g128trgb, SwitchOp("im128"));
	df.imwrite(im128, "i128-{it}", ImwriteOp("imw128"));

	auto m128 = df.multiplexer({ face_data_128, g128add }, MultiplexerOp("m128"));
	auto m128frgb = from_rgb(&df, m128, d_solver, fn, "m128frgb");
	
	auto d128 = conv(&df, m128frgb, d_solver, fn, fn, 3, 1, true, "d128"); // 128x128 -> 64x64
	auto m64 = df.multiplexer({ face_data_64, g64add }, MultiplexerOp("m64"));
	auto m64frgb = from_rgb(&df, m64, d_solver, fn, "m64frgb");
	auto m642 = df.multiplexer({ d128, m64frgb }, MultiplexerOp("m642"));

	auto d64 = conv(&df, m642, d_solver, fn, fn, 3, 1, true, "d64"); // 64x64 -> 32x32
	auto m32 = df.multiplexer({ face_data_32, g32add }, MultiplexerOp("m32"));
	auto m32frgb = from_rgb(&df, m32, d_solver, fn, "m32frgb");
	auto m322 = df.multiplexer({ d64, m32frgb }, MultiplexerOp("m322"));

	auto d32 = conv(&df, m322, d_solver, fn, fn, 3, 1, true, "d32"); // 32x32 -> 16x16
	auto m16 = df.multiplexer({ face_data_16, g16add }, MultiplexerOp("m16"));
	auto m16frgb = from_rgb(&df, m16, d_solver, fn, "m16frgb");
	auto m162 = df.multiplexer({ d32, m16frgb }, MultiplexerOp("m162"));

	auto d16 = conv(&df, m162, d_solver, fn, fn, 3, 1, true, "d16"); // 16x16 -> 8x8
	auto m8 = df.multiplexer({ face_data_8, g8trgb }, MultiplexerOp("m8"));
	auto m8frgb = from_rgb(&df, m8, d_solver, fn, "m8frgb");
	auto m82 = df.multiplexer({ d16, m8frgb }, MultiplexerOp("m82"));

	auto d8 = df.dense(m82, { fn * 8 * 8, 1, 1, 1 }, d_solver, DenseOp("d8"));	

	auto mloss = df.multiplexer({ face_labels, generator_labels }, MultiplexerOp("mloss"));
	
	std::string err;
	err = df.reduce_all(df.square_error(d8, mloss));
	auto loss = df.loss(err);
