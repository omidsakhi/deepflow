...
void Lifting::forward()
{
	...
	if (_mode == deepflow::LiftingParam_Mode_DOWN_REGULAR)
		LiftDownRegularKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
	else if (_mode == deepflow::LiftingParam_Mode_DOWN_FLIP)
		LiftDownFlipKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
	else if (_mode == deepflow::LiftingParam_Mode_UP_REGULAR)
		LiftUpRegularKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
	else if (_mode == deepflow::LiftingParam_Mode_UP_FLIP)
		LiftUpFlipKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
	else 
		LOG(FATAL);
	DF_KERNEL_CHECK();
}
...
void Lifting::backward()
{
	if (_inputs[0]->diff()) {
		...
		if (_mode == deepflow::LiftingParam_Mode_DOWN_REGULAR)
			LiftUpRegularKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _outputs[0]->diff()->gpu_data(), (float*)_inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
		else if (_mode == deepflow::LiftingParam_Mode_DOWN_FLIP)
			LiftUpFlipKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _outputs[0]->diff()->gpu_data(), (float*)_inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
		else if (_mode == deepflow::LiftingParam_Mode_UP_REGULAR)
			LiftDownRegularKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _outputs[0]->diff()->gpu_data(), (float*)_inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
		else if (_mode == deepflow::LiftingParam_Mode_UP_FLIP)
			LiftDownFlipKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, _outputs[0]->diff()->gpu_data(), (float*)_inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], 0);
		else
			LOG(FATAL);
		DF_KERNEL_CHECK();
	}
