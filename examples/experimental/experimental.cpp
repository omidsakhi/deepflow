...
void main(int argc, char** argv) {
	...
	if (FLAGS_i.empty()) {
		...
		auto stat = df.variable(df.ones({ 1,1,252,252 }),0);
		auto f = df.variable(df.random_uniform({ 1,1,5,5 },-1,1),solver);
		auto tconv = df.transposed_conv2d(stat, f, { 1,1,256,256 }, 0, 0, 1, 1, 1, 1);
		auto euc = df.euclidean_loss(tconv, image);		
		df.display(stat, 100, DisplayParam_DisplayType_DIFFS, "input", { "Train" });		
	}
	else {
