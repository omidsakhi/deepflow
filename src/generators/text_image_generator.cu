...
void TextImageGenerator::forward()
{
	...
	for (int index = 0; index < _n; ++index)
	{
		...
		*cv_img_target.get() = 0;
		...
		TextImageGeneratorKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, index * size, d_img_actual, _h, _w, (float*) _outputs[0]->value()->mutableData());
		DF_KERNEL_CHECK();
		TextImageGeneratorKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size, index * size, d_img_target, _h, _w, (float*)_outputs[1]->value()->mutableData());
		DF_KERNEL_CHECK();
	}
}
