...
BiasAdd::BiasAdd(const deepflow::NodeParam &_block_param) : Node(_block_param) {
	LOG_IF(FATAL, _block_param.has_bias_add_param() == false) << "param.has_bias_add_param() == false";
}
