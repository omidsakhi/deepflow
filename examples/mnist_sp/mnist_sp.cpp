...
void create_graph(DeepFlow *df) {
	...
	auto imwrite_input_28 = df->place_holder({ FLAGS_batch, 1, 28, 28 }, PlaceholderOp("imwrite_input_28"));
	df->imwrite(imwrite_input_28, "{it}", ImwriteOp("imwrite_28"));
	...
	auto input = df->place_holder({ FLAGS_batch, 1, 28, 28 }, PlaceholderOp("input"));

	auto loc = df->conv2d(input, 1, fn, solver, ConvolutionOp("l14").kernel(3).stride(2).with_bias());
	loc = df->leaky_relu(loc);

	loc = df->conv2d(loc, fn, fn * 2, solver, ConvolutionOp("l7").kernel(3).stride(2).with_bias());
	loc = df->leaky_relu(loc);

	loc = df->conv2d(loc, fn * 2, fn * 4, solver, ConvolutionOp("l4").kernel(3).stride(2).with_bias());
	loc = df->leaky_relu(loc);
	
	loc = df->dropout(loc);

	auto theta = df->dense(loc, { fn * 4 * 4 * 4, 1, 2, 3 }, solver, DenseOp("theta"));	

	auto net_input = df->spatial_transformer(input, theta, FLAGS_batch, 28, 28, solver, SpatialTransformerOp("sp_out"));

	auto net = df->conv2d(net_input, 1, fn, solver, ConvolutionOp("conv1").kernel(3).pad(1).stride(2).with_bias()); // 14x14
...
void main(int argc, char** argv) {
	...
	auto sp_out = session->get_node("sp_out");
	auto imwrite_input_28 = session->get_placeholder("imwrite_input_28");
	auto imwrite_28 = session->get_node<ImageWriter>("imwrite_28");
