...
EuclideanLoss::EuclideanLoss(const deepflow::NodeParam &_block_param) : Loss(_block_param) {
	LOG_IF(FATAL, _block_param.loss_param().has_euclidean_loss_param() == false) << "param.loss_param().has_euclidean_loss_param() == false";
}
