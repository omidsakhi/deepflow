...
void TransposedConvolution2D::initForward() {	
	...
	LOG_IF(FATAL, cudnnSetConvolution2dDescriptor(_convDesc, pad_h, pad_w, u, v, dilation_h, dilation_w, CUDNN_CROSS_CORRELATION, CUDNN_DATA_FLOAT) != 0);
	...
	int n, c, h, w;
	LOG_IF(FATAL, cudnnGetConvolution2dForwardOutputDim(_convDesc, _outputs[0]->value()->descriptor(), _filterDesc, &n, &c, &h, &w) != 0);
	...
	LOG_IF(FATAL, cudnnGetConvolutionForwardAlgorithm(_cudnnHandle, _outputs[0]->value()->descriptor(), _filterDesc, _convDesc, _inputs[0]->value()->descriptor(), CUDNN_CONVOLUTION_FWD_PREFER_FASTEST, 0, &_fwdAlgo) != 0);
	LOG_IF(FATAL, cudnnGetConvolutionForwardWorkspaceSize(_cudnnHandle, _outputs[0]->value()->descriptor(), _filterDesc, _convDesc, _inputs[0]->value()->descriptor(), _fwdAlgo, &_fwdWorkspaceSize) != 0);		
	LOG_IF(FATAL, cudnnGetConvolutionBackwardDataAlgorithm(_cudnnHandle, _filterDesc, _inputs[0]->value()->descriptor(), _convDesc, _outputs[0]->value()->descriptor(), CUDNN_CONVOLUTION_BWD_DATA_PREFER_FASTEST, 0, &_bwdDataAlgo) != 0) << "cudnnGetConvolutionBackwardDataAlgorithm [FAILED]";
	LOG_IF(FATAL, cudnnGetConvolutionBackwardDataWorkspaceSize(_cudnnHandle, _filterDesc, _inputs[0]->value()->descriptor(), _convDesc, _outputs[0]->value()->descriptor(), _bwdDataAlgo, &_bwdDataWorkspaceSize) != 0) << "cudnnGetConvolutionBackwardDataWorkspaceSize [FAILED]";
...
void TransposedConvolution2D::forward() {
	...
	cudnnStatus_t t;
	t = cudnnConvolutionBackwardData(_cudnnHandle, &alpha, _filterDesc, _inputs[1]->value()->data(), _inputs[0]->value()->descriptor(), _inputs[0]->diff()->data(), _convDesc, _bwdDataAlgo, d_workspace, _bwdDataWorkspaceSize, &beta, _outputs[0]->value()->descriptor(), _outputs[0]->value()->mutableData());
	LOG_IF(FATAL, t != 0) << "forward() - cudnnConvolutionBackwardData [FAILED] - " << cudnnGetErrorString(t);
}

void TransposedConvolution2D::backward() {
	cudnnStatus_t t;
	t = cudnnConvolutionForward(_cudnnHandle, &alpha, _inputs[0]->value()->descriptor(), _inputs[0]->value()->data(), _filterDesc, _inputs[1]->value()->data(), _convDesc, _fwdAlgo, d_workspace, _fwdWorkspaceSize, &beta, _outputs[0]->value()->descriptor(), _outputs[0]->value()->mutableData());
	LOG_IF(FATAL, t != 0) << "backward() - cudnnConvolutionForward [FAILED]";
}
