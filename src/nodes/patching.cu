...
void Patching::forward()
{
	...
	if (_mode == deepflow::PatchingParam_Mode_DOWNSAMPLES || _mode == deepflow::PatchingParam_Mode_DOWNCHANNELS) {
		...
		PatchingDownKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size,  
			...
			_inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], patch_h, patch_w, 0);
	}
	else
		PatchingUpKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size,
			_mode == deepflow::PatchingParam_Mode_UPSAMPLES,
		_inputs[0]->value()->gpu_data(), (float*)_outputs[0]->value()->gpu_data(), dims[0], dims[1], dims[2], dims[3], _num_vertical_patches, _num_horizontal_patches, 0);
	DF_KERNEL_CHECK();
}
...
void Patching::backward()
{
	if (_inputs[0]->diff()) {
		...
		if (_mode == deepflow::PatchingParam_Mode_DOWNSAMPLES || _mode == deepflow::PatchingParam_Mode_DOWNCHANNELS)
			PatchingUpKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size,
				_mode == deepflow::PatchingParam_Mode_DOWNSAMPLES,
				_outputs[0]->diff()->gpu_data(), _inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], _num_vertical_patches, _num_horizontal_patches, 0);
		else {
			int patch_h = dims[2] / _num_vertical_patches;
			int patch_w = dims[3] / _num_horizontal_patches;
			PatchingDownKernel << < numOfBlocks(size), maxThreadsPerBlock >> > (size,
				_mode == deepflow::PatchingParam_Mode_UPSAMPLES,
				_outputs[0]->diff()->gpu_data(), _inputs[0]->diff()->gpu_data(), dims[0], dims[1], dims[2], dims[3], patch_h, patch_w, 0);
		}
		DF_KERNEL_CHECK();
	}
}
